...
def linear_extrap(correlation):
    print('performing linear extrapolation...')
    num_step, N = correlation.shape
    extend_time = num_step
    p = num_step // 2
    new_corr = np.zeros([num_step*2, N], dtype=np.complex)
    for idx in range(N):
        # time slice of the correlation function.
        corr_time = correlation[:,idx]
        a_vec_1 = lpc_ref(corr_time, p)
        for j in range(extend_time):
            corr_time = np.concatenate((corr_time, np.array([-np.dot(a_vec_1[1:], corr_time[-p:][::-1])])))

        new_corr[:, idx] = corr_time
...
if __name__ == '__main__':
    print('Hello')
    correlation = np.load('DSF/kitaev_honeycomb_dt_0.20_var_1/S_xt_L7_D6_Dc12_A.npy')
    new_corr = linear_extrap(correlation)
    import matplotlib.pyplot as plt
    plt.plot(new_corr[:,48].real)
    plt.show()
